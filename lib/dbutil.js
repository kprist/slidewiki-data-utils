...
async function connect(db, host='localhost', port=27017) {
    // Initialize connection once
    let database = await MongoClient.connect(`mongodb://${host}:${port}/${db}`);
    console.log(`Connected to mongo instance ${host}:${port}/${db}`);
    return database;
}
